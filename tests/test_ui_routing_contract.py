...
class UiRoutingContractTests(unittest.TestCase):
    ...
    def test_ai_portfolio_ui_module_contract(self):
        ...
        self.assertIn('href="styles.css?v=20260523-python-strategy-v13"', html)
        self.assertIn('src="app.js?v=20260523-python-strategy-v13"', html)
        ...
        self.assertIn("compactIdentifier", module_source)
    ...
    def test_cross_dashboard_smoke_tracks_current_bundle_and_quantamental(self):
        ...
        self.assertIn('APP_BUNDLE_VERSION = "20260523-python-strategy-v13"', smoke_source)
    ...
    def test_ml_forecast_registry_actions_preserve_action_name(self):
        ...
        self.assertIn("function compactIdentifier", self.source)
        self.assertIn("compactIdentifierCell", self.source)
    ...
    def test_strategy_editor_is_code_only_not_universe_editor(self):
        ...
        self.assertIn("function strategyGenerationParameterTuningFromControls", self.source)
        ...
        self.assertIn('data-testid="auto-trading-generate">JSON 전략</button>', html)
    ...
    def test_macro_static_ui_contract(self):
        ...
        for marker in [
            ...
            'data-panel-view="all"',
            '<section class="home-card market-overview-card market-surface" data-panel-tier="primary">',
        ...
        self.assertIn(".quality-detail-list", css)
        ...
        self.assertIn("qualityFetchJson(url, timeoutMs = QUALITY_DASHBOARD_TIMEOUT_MS)", self.source)
        self.assertIn("function forecastQualityPayload", self.source)
        self.assertIn("function renderQualityForecastState", self.source)
        self.assertIn("function renderQualityAiPortfolioState", self.source)
        self.assertIn("forecastDatasetPreview: null", self.source)
        self.assertIn("forecastAiProviderStatus: null", self.source)
        self.assertIn("model_availability", self.source)
        ...
        self.assertIn("els.dashboardViewControls.hidden = false", self.source)
        self.assertIn("els.homeSurfaceGrid.dataset.panelView = normalized", self.source)
        self.assertNotIn("dashboardPanelView === \"market\"", self.source)
    ...
    def test_auto_trading_tab_owns_strategy_operation_surfaces(self):
        ...
        self.assertIn('data-testid="python-strategy-run"', html)
        self.assertIn('data-testid="python-strategy-lab"', html)
        self.assertIn('id="pythonStrategyCode"', html)
        ...
        self.assertIn('async function runPythonStrategyLab', self.source)
        self.assertIn('API.quantPythonStrategyRun', self.source)
        self.assertIn('function renderPythonStrategyChart', self.source)
        self.assertIn('function renderPythonStrategyIndicatorPanel', self.source)
        self.assertIn('function renderPythonStrategyTradePaths', self.source)
        self.assertIn('function renderPythonStrategyExplanation', self.source)
        self.assertIn('function renderPythonStrategyOptimizationVisual', self.source)
        self.assertIn('function renderPythonStrategyRobustness', self.source)
        ...
        self.assertIn('.python-supertrend-line', css)
        self.assertIn('.python-fast-ma-line', css)
        self.assertIn('.python-slow-ma-line', css)
        self.assertIn('.python-rsi-line', css)
        self.assertIn('.python-optimization-visual', css)
        self.assertIn('.python-robustness-validation', css)
        self.assertIn('.python-walk-forward-grid', css)
        self.assertIn('.python-trade-path', css)
        self.assertIn('.python-position-band', css)
        self.assertIn('.python-trial-score-strip', css)
        self.assertIn('grid-template-areas:\n    "controls editor"\n    "results results";', css)
        self.assertIn('.auto-trading-left {\n  grid-area: controls;', css)
        self.assertIn('.auto-trading-editor-stack {\n  grid-area: editor;', css)
        self.assertIn('.auto-trading-output-stack {\n  grid-area: results;', css)
        self.assertIn('.auto-trading-editor-stack > .auto-trading-prompt-panel', css)
        self.assertIn('.auto-trading-output-stack > #pythonStrategyResultSurface', css)
        self.assertIn('.auto-trading-output-stack > #autoTradingChartSurface', css)
        self.assertIn('data-testid="python-strategy-indicator-panel"', self.source)
        self.assertIn('data-testid="python-strategy-trade-path"', self.source)
        self.assertIn('data-testid="python-strategy-explanation"', self.source)
        self.assertIn('data-testid="python-strategy-optimization-visual"', self.source)
        self.assertIn('data-testid="python-strategy-robustness-validation"', self.source)
        self.assertIn('data-testid="python-strategy-oos-validation"', self.source)
        self.assertIn('data-testid="python-strategy-walk-forward"', self.source)
        self.assertIn('data-testid="python-strategy-cost-stress"', self.source)
        self.assertIn('data-testid="python-strategy-monte-carlo"', self.source)
    ...
    def test_purpose_layout_orders_market_quantamental_and_forecast(self):
        css = STYLES_CSS.read_text(encoding="utf-8")

        def assert_class_order(tab: str, class_name: str, order: str) -> None:
            pattern = rf'\.dashboard-surface-grid\[data-dashboard-tab="{tab}"\] \.{class_name} \{{(?P<body>.*?)\}}'
            matches = list(re.finditer(pattern, css, re.S))
            self.assertTrue(matches, f"{tab}:{class_name}")
            self.assertIn(f"order: {order};", matches[-1].group("body"))

        def assert_id_order(tab: str, element_id: str, order: str) -> None:
            pattern = rf'\.dashboard-surface-grid\[data-dashboard-tab="{tab}"\] #{element_id} \{{(?P<body>.*?)\}}'
            matches = list(re.finditer(pattern, css, re.S))
        ...
        expected_quantamental = {
            ...
            "top5": ("class", "quantamental-screen-card", "2"),
            "scoreThreshold": ("class", "quantamental-score-screen-card", "3"),
            "signal": ("class", "quantamental-signal-card-shell", "4"),
            "score": ("class", "quantamental-score-card", "5"),
            "factor": ("class", "quantamental-factor-card", "6"),
            "aiReport": ("class", "quantamental-main-card", "7"),
            "quality": ("class", "quantamental-quality-card", "8"),
            "compare": ("class", "quantamental-compare-card", "9"),
        }
        for _, (selector_type, selector, order) in expected_quantamental.items():
            if selector_type == "id":
                assert_id_order("quantamental", selector, order)
            else:
