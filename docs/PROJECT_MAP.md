...
- `app/web/app.js` and `app/web/styles.css`: static Risk tab renderers and styles, including the decision-brief readiness radar, workflow lane, causal path map, coverage topology, evidence trace map, service gate rail, plus first-class driver waterfall, transmission-flow, and scenario-heatmap visualizations over the typed Risk response.
