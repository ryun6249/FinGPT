...
- The static UI renders existing Risk contract fields into visual decision surfaces: the first-flow decision brief shows a readiness radar, workflow lane, causal path map, pressure stack, coverage topology, evidence trace map, and service gate rail, while the evidence drawer mirrors the pressure/topology/trace controls and the detailed sections show driver contributions as SVG bars, transmission channels as a flow map, and scenario stress rows as a heatmap before the tables/cards. These are presentation-only views over deterministic contract data.
